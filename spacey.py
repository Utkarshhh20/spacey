...
over_theme = {'txc_inactive': "#D3D3D3", 'menu_background': '#3948A5', 'txc_active': 'white',
              'option_active': '#3948A5'}
...
if dashboard == 'Asteroid Defense':
    ...
    colc.markdown(
        """*This project was inspired to emulate a simplified model of a Falcon-9. This ended up being one of our team's most favorite projects as it utilized applied physics and programming""",
...
if dashboard == 'Create your own galaxy':
    ...
    tot_stars = st.slider("Number of stars in galaxy", 1000, 6000, 3000)
    size = st.slider("Number of turns in galaxy", 10, 150, 20)
    ...
    points_per_turn = tot_stars / size

    for curr_point_num in range(tot_stars):
        curr_turn, i = divmod(curr_point_num, points_per_turn)
        angle = (curr_turn + 1) * 2 * math.pi * i / points_per_turn
        radius = curr_point_num / tot_stars
    ...
    condition = st.selectbox('Choose the conditions: ',
                             options=['Cloudy', 'Fair', 'Heavy T-Storm', 'Light rain', 'Mostly Cloudy', 'Partly Cloudy',
                                      'T-Storm', 'Rain', 'Partly Cloudy', 'Thunder'])
    if condition == 'Cloudy':
        condition = 0
    elif condition == 'Fair':
        condition = 1
    ...
    if button == True:
        data_input = [crew, hightemp, lowtemp, avgtemp, launchtemp, histhightemp, histlowtemp, histavgtemp,
                      precipitation, histavgprecipitation, winddir, maxwind, vis, windlaunch, condition]
